...
function _checkCoin(coin) {
  if (!_.includes(['btc', 'bch', 'darkpay'], coin)) throw new Error('Invalid coin');
};
...
Credentials._getNetworkFromExtendedKey = function(xKey, coin) {
  ...
  if (coin == 'darkpay') return xKey.charAt(0) == 'x' ? 'testnet' : 'livenet';
